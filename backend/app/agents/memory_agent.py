...
def _looks_like_stable_persona_claim(value: str) -> bool:
...
class MemoryExtractor:
    ...
    def post_process(
        ...
        for candidate in result.candidates or []:
            ...
            # A target-role quote in a simulation session was generated by the
            # model. It may preserve turn continuity, but it is never evidence
            # for a stable real-person trait.
            if candidate.evidence_role == "target":
                if _looks_like_stable_persona_claim(candidate.content):
                    continue
                if candidate.category == "target_sensitive_point":
                    candidate.category = "focus_issue"
                    candidate.content = _truncate(
                        f"本次模拟中暂时需要确认：{candidate.content}",
                        240,
                    )
                if candidate.confidence == "high":
                    candidate.confidence = "medium"
                candidate.tags = _dedupe_texts(
                    [*candidate.tags, "模拟回复事件", "非人物画像证据"],
                    limit=6,
                    item_limit=40,
                )
...
class MemoryManager:
    ...
    def update(
        ...
        *,
        ...
        memory = self._sanitize_previous_memory(self._ensure_memory(request))
    ...
    def _sanitize_previous_memory(self, memory: SessionMemory) -> SessionMemory:
        """Migrate old target-derived persona claims into safe session scope."""

        safe_items: list[MemoryItem] = []
        for item in memory.memory_items:
            target_only = bool(item.evidence) and all(
                evidence.role == "target" for evidence in item.evidence
            )
            if target_only and _looks_like_stable_persona_claim(item.content):
                continue
            if target_only and item.category == "target_sensitive_point":
                item.category = "focus_issue"
                item.confidence = "low"
                item.tags = _dedupe_texts(
                    [*item.tags, "模拟回复事件", "非人物画像证据"],
                    limit=8,
                    item_limit=40,
                )
                for evidence in item.evidence:
                    evidence.source_type = "SIMULATED_TARGET_REPLY"
            safe_items.append(item)
        memory.memory_items = safe_items
        return memory
