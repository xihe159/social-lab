...
class SessionOrchestrator:
    ...
    当前流程：
    1. SafetyAgent：检查隐私、操控、骚扰、威胁等风险；
    2. SimulationAgent：生成目标人物回复；
    3. StateAgent：更新对话 Dynamics；
       - V1 同时校正关系状态；
       - V2 保留 SimulationAgent V2 的关系状态结果；
    ...
    async def handle_message(
        ...
        *,
        ...
        """
        处理一轮用户消息，并返回目标人物回复、状态变化、
        Dynamics、记忆更新和安全检查结果。
        """
        ...
        if self._should_block(safety_result):
            ...
            logger.info(
                ...
                extra={
                    ...
                    "has_updated_memory": (
                        blocked_response.updated_memory is not None
                    ),
                    "has_dynamics_update": False,
                },
            )
        ...
        # 先保留 SimulationAgent 的初始状态判断，
        # 作为 StateAgent 失败时的兜底。
        state_delta = simulation_response.simulation.state_delta
        risk_flags = list(simulation_response.simulation.risk_flags)

        # V1：StateAgent 同时校正关系状态和 Dynamics。
        # V2：SimulationAgent V2 保留关系状态控制权，
        #     StateAgent 只负责 Dynamics 与补充风险信号。
        ...
        simulation_response.updated_memory = (
            await self._run_memory_agent_with_fallback(
                trace_id=trace_id,
                request=request,
                simulation_response=simulation_response,
        ...
        logger.info(
            ...
            extra={
                ...
                "reply_length": len(
                    simulation_response.target_message.content
                ),
                "has_updated_memory": (
                    simulation_response.updated_memory is not None
                ),
                "has_dynamics_update": (
                    simulation_response.dynamics_update is not None
                ),
            },
        )
    ...
    async def _run_simulation_agent(
        ...
        *,
        ...
        # 把 SafetyAgent 检查结果合并进主响应。
        simulation_response.safety = safety_result

        # 如果 SafetyAgent 给出 warn/rewrite，合并到 risk_flags。
        ...
        logger.info(
            ...
            extra={
                ...
                "state_delta": (
                    simulation_response.simulation.state_delta.model_dump()
                ),
                "risk_flags": (
                    simulation_response.simulation.risk_flags
                ),
                "updated_state": (
                    simulation_response.updated_state.model_dump()
                ),
                "simulation_evaluated": bool(
                    evaluation_meta and evaluation_meta.evaluated
                ),
                "initial_evaluation_score": (
                    evaluation_meta.initial_score
                    if evaluation_meta
                    else None
                ),
                "final_evaluation_score": (
                    evaluation_meta.final_score
                    if evaluation_meta
                    else None
                ),
                "evaluation_score_delta": (
                    evaluation_meta.score_delta
                    if evaluation_meta
                    else None
                ),
                "evaluation_verdict": (
                    ...
                    if (
                        evaluation_meta
                        and evaluation_meta.final_verdict
                    )
                    else None
                ),
                "failure_attribution": (
                ...
                "feedback_retry_count": (
                    evaluation_meta.retry_count
                    if evaluation_meta
                    else 0
                ),
                "evaluation_agent_failed": bool(
                    evaluation_meta
                    and evaluation_meta.evaluator_failed
                ),
                "final_evaluation_failed": bool(
                    evaluation_meta
                    and evaluation_meta.final_evaluator_failed
                ),
                "decision_fallback_used": bool(
                    runtime_meta
                    and runtime_meta.decision_fallback_used
                ),
                "strategy_policy_id": (
                    strategy_meta.policy_id
                    if strategy_meta
                    else None
                ),
                "strategy_action": (
                    strategy_meta.strategy_action
                    if strategy_meta
                    else None
                ),
                "strategy_confidence": (
                    strategy_meta.confidence
                    if strategy_meta
                    else None
                ),
                "strategy_fallback_used": bool(
                    runtime_meta
                    and runtime_meta.strategy_fallback_used
                ),
                "generator_retry_count": (
                    runtime_meta.generator_retry_count
                    if runtime_meta
                    else 0
                ),
                "generator_fallback_used": bool(
                    runtime_meta
                    and runtime_meta.generator_fallback_used
                ),
                "evaluation_call_count": (
                    runtime_meta.evaluation_call_count
                    if runtime_meta
                    else 0
                ),
                ...
                "evaluation_background_scheduled": bool(
                    evaluation_meta
                    and evaluation_meta.background_scheduled
                ),
                "evaluation_critical_reasons": (
                    evaluation_meta.critical_reasons
                    if evaluation_meta
                    else []
                ),
                "strategy_replan_count": (
                    runtime_meta.strategy_replan_count
                    if runtime_meta
                    else 0
                ),
                "simulation_revision_count": (
                    runtime_meta.simulation_revision_count
                    if runtime_meta
                    else 0
                ),
                "rejected_candidate_discarded": bool(
                    runtime_meta
                    and runtime_meta.rejected_candidate_discarded
                ),
            },
        )
        ...
        *,
        ...
        logger.info(
            ...
            extra={
                ...
                "has_current_dynamics": (
                    request.current_dynamics is not None
                ),
            },
        )
        ...
        try:
            state_request = StateEvaluateRequest(
                ...
                simulation_attitude=(
                    simulation_response.simulation.attitude
                ),
                simulation_emotion=(
                    simulation_response.simulation.emotion
                ),
                perceived_user_tone=(
                    simulation_response.simulation.perceived_user_tone
                ),
            ...
            state_evaluation = await self.state_agent.run(
                state_request
            )
            ...
            if override_relationship_state:
                # V1 使用 StateAgent 重新评估的关系增量。
                state_delta = state_evaluation.state_delta
                simulation_response.simulation.state_delta = (
                    state_delta
                )
                ...
                # V2 的关系状态由 SimulationAgent V2 决策链全权负责。
                state_delta = fallback_state_delta

            simulation_response.simulation.risk_flags = risk_flags
            simulation_response.dynamics_update = (
                state_evaluation.dynamics_update
            )
            ...
            logger.info(
                ...
                extra={
                    ...
                    "dynamics_delta": (
                        state_evaluation.dynamics_update
                        .dynamics_delta.model_dump()
                    ),
                    "updated_dynamics": (
                        state_evaluation.dynamics_update
                        .updated_dynamics.model_dump()
                    ),
                },
            )
            ...
            logger.exception(
                ...
                extra={
                    ...
                    "fallback": (
                        "use_simulation_agent_state_delta"
                    ),
                },
            )
    ...
    async def _run_memory_agent_with_fallback(
        ...
        *,
        ...
        try:
            ...
            memory_response = await self.memory_agent.run(
                memory_request
            )

            memory_reason = getattr(
                memory_response,
                "memory_reason",
                "",
            )
            ...
            logger.info(
                ...
                extra={
                    ...
                    "status": "success",
                    "has_memory": (
                        memory_response.memory is not None
                    ),
